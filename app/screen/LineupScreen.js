...
const LineupScreen = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (schedData && schedData.length > 0) {
        ...
        // 멤버 정보 및 내 팀 ID 가져오기
        const memRes = await fetch(`${API_BASE_URL}/api/member`, { signal: controller.signal });
        ...
        const memberStatusColumn = homeSide ? currentSched.home_member : currentSched.away_member;
        ...
        if (Array.isArray(memberStatus)) {
          ...
          attendingIds = Object.keys(memberStatus).filter(mid => 
            memberStatus[mid] === 1 || memberStatus[mid] === "1"
          );
        }
        ...
        const rawLineupData = homeSide ? currentSched.home_lineup : currentSched.away_lineup;
        if (rawLineupData) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/schedule/${targetDate}/lineup?side=${side}`, {
        method: "POST",
    ...
    setLineup(prev => {
      ...
      if (activeTab === 'defense') {
        ...
        if (!isBench) {
            ...
            newLineup.defense.BENCH = (newLineup.defense.BENCH || []).filter(id => id?.toString() !== sId);
            ...
            if (currentBench.some(id => id?.toString() === sId)) {
                newLineup.defense.BENCH = currentBench.filter(id => id?.toString() !== sId);
            } else {
      ...
      // 2. 타순 설정 모드일 때
        ...
        const isDefender = Object.keys(newLineup.defense).some(k => k !== 'BENCH' && newLineup.defense[k]?.toString() === sId);
        ...
        newLineup.batting = newLineup.batting.map(id => id?.toString() === sId ? null : id);
  ...
  const handleAutoBench = () => {
    setLineup(prev => {
      ...
      attendees.forEach(member => {
          const mId = member.Id ? Number(member.Id) : member.User_ID;
          if (mId && !currentAssignedIds.has(mId.toString()) && !newLineup.defense.BENCH.some(bid => bid?.toString() === mId.toString())) {
              newLineup.defense.BENCH.push(mId);
          }
      });
  ...
  const isMemberAssigned = (member) => {
    ...
    if (activeTab === 'defense') {
        const isPos = Object.keys(lineup.defense).filter(k => k !== 'BENCH').some(k => lineup.defense[k]?.toString() === sId);
        const isBench = (lineup.defense.BENCH || []).some(bid => bid?.toString() === sId);
  ...
  const getAssignedKey = (member) => {
    ...
    if (activeTab === 'defense') {
        const pos = Object.keys(lineup.defense).find(key => key !== 'BENCH' && lineup.defense[key]?.toString() === sId);
        if (pos) return pos;
        if ((lineup.defense.BENCH || []).some(bid => bid?.toString() === sId)) return "후보";
        return null;
    } else {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.fieldSection}>
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContainer}>
                  ...
                  <View style={styles.posP}><PositionSlot pos="P" name={getNameById(lineup.defense.P)} stadium /></View>
                  <View style={styles.posC}><PositionSlot pos="C" name={getNameById(lineup.defense.C)} stadium /></View>
                  <View style={styles.pos1B}><PositionSlot pos="1B" name={getNameById(lineup.defense["1B"])} stadium /></View>
                  <View style={styles.pos2B}><PositionSlot pos="2B" name={getNameById(lineup.defense["2B"])} stadium /></View>
                  <View style={styles.pos3B}><PositionSlot pos="3B" name={getNameById(lineup.defense["3B"])} stadium /></View>
                  <View style={styles.posSS}><PositionSlot pos="SS" name={getNameById(lineup.defense.SS)} stadium /></View>
                  <View style={styles.posLF}><PositionSlot pos="LF" name={getNameById(lineup.defense.LF)} stadium /></View>
                  <View style={styles.posCF}><PositionSlot pos="CF" name={getNameById(lineup.defense.CF)} stadium /></View>
                  <View style={styles.posRF}><PositionSlot pos="RF" name={getNameById(lineup.defense.RF)} stadium /></View>
                  
                  {/* 지명타자(DH) 배치 */}
                  <View style={styles.posDH}><PositionSlot pos="DH" name={getNameById(lineup.defense.DH)} stadium /></View>
              </View>
            </View>
          </View>
        ) : (
            ...
            <View style={styles.battingList}>
              ...
              {BATTING_ORDERS.map((order, idx) => (
                ...
                >
                  ...
                  {lineup.batting[idx] ? (
                    <Text style={styles.battingName}>{getNameById(lineup.batting[idx])}</Text>
                  ) : (
        ...
        <View style={styles.rosterSection}>
          ...
          {attendees.map(member => {
            const mId = member.Id ? Number(member.Id) : member.User_ID;
            const sId = mId?.toString();
            const isDefender = Object.keys(lineup.defense).some(k => k !== 'BENCH' && lineup.defense[k]?.toString() === sId);
                  ...
                  {activeTab === 'defense' ? (
                    POSITIONS.map(pos => {
                      const isActive = pos === 'BENCH' 
                        ? (lineup.defense.BENCH || []).some(bid => bid?.toString() === sId) 
                    ...
                    <TouchableOpacity 
                      ...
                      style={[
                        styles.posBtn, 
                        lineup.batting.some(id => id?.toString() === sId) && styles.posBtnActive, 
                    ...
                    >
                      <Text style={[
                        styles.posBtnText, 
                        lineup.batting.some(id => id?.toString() === sId) && styles.posBtnTextActive, 
...
const PositionSlot = ({ pos, name, highlight, stadium }) => (
  <View style={[
    ...
    <Text style={[styles.slotPos, stadium && styles.slotPosStadium]}>{pos}</Text>
    <Text style={[styles.slotName, stadium && styles.slotNameStadium]} numberOfLines={1}>{name || "---"}</Text>
  </View>
);
