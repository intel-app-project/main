...
const normalizeSchedule = (row, teamNameMap, scoreMap = {}, managedTeamId) => {
  ...
  const isManagedGame =
    String(home ?? "") === String(managedTeamId ?? "") ||
    String(away ?? "") === String(managedTeamId ?? "");

  return {
    home: getTeamName(teamNameMap, home, "HOME"),
    away: getTeamName(teamNameMap, away, "AWAY"),
    scheduleDate: String(dateValue || ""),
    isManagedGame,
    homeScore:
...
const DirectorScheduleScreen = ({ navigation, route }) => {
  ...
  const [reviewSendingKey, setReviewSendingKey] = useState("");
  ...
  useEffect(() => {
    ...
    const loadSchedules = async () => {
      try {
        ...
        if (!currentTeamId) {
          throw new Error("Failed to find director team.");
        }
        ...
        const normalizedGames = (
          ...
          .map((row) =>
            normalizeSchedule(row, teamNameMap, scoreMap, currentTeamId),
          )
          .filter(Boolean);
        ...
        if (isMounted) {
          setManagedTeamId(currentTeamId);
        ...
        console.error("[DirectorScheduleScreen] load failed", error);
        if (isMounted) {
  ...
  const handleSendReview = async (game) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(result?.detail || "review generate failed");
      }
                          ...
                          <View style={styles.scheduleItemRow}>
                            <View style={styles.scheduleMain}>
                              ...
                              <View style={styles.scheduleDateRow}>
                                ...
                                {activeTab === "PAST" && game.isManagedGame ? (
                                  <TouchableOpacity
                                    style={lineupButtonStyle}
                                    onPress={() => handleSendReview(game)}
                                    disabled={
                                      reviewSendingKey === game.scheduleDate
                                    }
                                  >
                                    <Text style={lineupButtonTextStyle}>
                                      {reviewSendingKey === game.scheduleDate
