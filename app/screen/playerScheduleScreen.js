...
const PlayerScheduleScreen = ({ route }) => {
    ...
    const load = async () => {
      try {
        ...
        if (memberRes.ok) {
          ...
          const allMemberRes = await fetch(`${API_BASE_URL}${MEMBER_API_ENDPOINT}`);
          if (!allMemberRes.ok) throw new Error(`member API error: ${allMemberRes.status}`);
          const members = await allMemberRes.json();
          if (Array.isArray(members)) {
            for (let i = 0; i < members.length; i += 1) {
              if (
                String(members[i]?.Id ?? "").trim() === String(id ?? "").trim() ||
                String(members[i]?.User_ID ?? "").trim() === String(id ?? "").trim()
              ) {
                member = members[i];
                break;
              }
        ...
        if (Array.isArray(teamRows)) {
          for (let i = 0; i < teamRows.length; i += 1) {
            teamNameMap[String(teamRows[i]?.id ?? "")] = String(teamRows[i]?.name ?? "");
          }
        }
        ...
        if (Array.isArray(scheduleRows)) {
          for (let i = 0; i < scheduleRows.length; i += 1) {
            ...
            let date = null;
            if (typeof row?.date === "string" && /^\d{8}$/.test(row.date.trim())) {
              date = new Date(
            ...
            nextList.push({
              ...
              isHome,
              attendanceSide: side,
              attendanceStatus: value === 1 ? "attending" : value === 0 ? "absent" : "pending",
              date,
              ...
              teamName: isHome
                ? teamNameMap[String(row?.home ?? "")] || `TEAM ${row?.home ?? ""}`
                : teamNameMap[String(row?.away ?? "")] || `TEAM ${row?.away ?? ""}`,
              opponent: isHome
                ? teamNameMap[String(row?.away ?? "")] || `TEAM ${row?.away ?? ""}`
                : teamNameMap[String(row?.home ?? "")] || `TEAM ${row?.home ?? ""}`,
            });
        ...
        if (mounted) {
          ...
          if (nextList.length > 0) {
            setMonthDate(new Date(nextList[0].date.getFullYear(), nextList[0].date.getMonth(), 1));
          } else {
        ...
        if (mounted) setErrorText(error.message || "일정을 불러오지 못했습니다.");
      } finally {
  ...
  const visibleSchedules = useMemo(() => scheduleList.slice(0, visibleCount), [scheduleList, visibleCount]);

  const runMonth = (offset) => {
    const direction = offset > 0 ? -1 : 1;
    Animated.parallel([
      Animated.timing(translateX, { toValue: 18 * direction, duration: 120, useNativeDriver: true }),
  ...
  const updateAttendance = async (item, status) => {
    try {
      ...
      if (!response.ok) throw new Error(result?.detail || `attendance API error: ${response.status}`);

      setScheduleList((current) => current.map((game) => (game.key === item.key ? { ...game, attendanceStatus: status } : game)));
      setOpenedKey("");
    } catch (error) {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.body}>
        <View style={styles.calendarSection}>
          <View style={[styles.card, styles.calendarCard]} {...panResponder.panHandlers}>
            <Text style={styles.cardEyebrow}>Schedule Calendar</Text>
            <View style={styles.calendarTopRow}>
              <Text style={styles.cardTitle}>{`${monthDate.getFullYear()}년 ${monthDate.getMonth() + 1}월`}</Text>
              <TouchableOpacity
            ...
            <View style={styles.calendarBody}>
              <TouchableOpacity style={[styles.calendarSideButton, styles.calendarSideButtonLeft]} onPress={() => runMonth(-1)}>
              ...
              <Animated.View style={{ transform: [{ translateX }], opacity }}>
                ...
                {calendarRows.map((row, rowIndex) => (
                  <View key={`row-${rowIndex}`} style={styles.weekRow}>
                    {row.map((cell) => (
                      <View key={`${cell.getFullYear()}-${cell.getMonth()}-${cell.getDate()}`} style={styles.dayCell}>
                        <View
                          style={[
                            styles.dayCircle,
                            scheduleList.some((game) => game.date.getFullYear() === cell.getFullYear() && game.date.getMonth() === cell.getMonth() && game.date.getDate() === cell.getDate()) && styles.dayEvent,
                            scheduleList.some((game) => game.date.getFullYear() === cell.getFullYear() && game.date.getMonth() === cell.getMonth() && game.date.getDate() === cell.getDate() && game.attendanceStatus === "attending") && styles.dayEventAttending,
                            scheduleList.some((game) => game.date.getFullYear() === cell.getFullYear() && game.date.getMonth() === cell.getMonth() && game.date.getDate() === cell.getDate() && game.attendanceStatus === "pending") && styles.dayEventPending,
                            scheduleList.some((game) => game.date.getFullYear() === cell.getFullYear() && game.date.getMonth() === cell.getMonth() && game.date.getDate() === cell.getDate() && game.attendanceStatus === "absent") && styles.dayEventAbsent,
                            new Date().getFullYear() === cell.getFullYear() && new Date().getMonth() === cell.getMonth() && new Date().getDate() === cell.getDate() && styles.dayToday,
                          ]}
                        >
                          <Text style={[styles.dayText, cell.getMonth() !== monthDate.getMonth() && styles.dayDim]}>{cell.getDate()}</Text>
                        </View>
                      </View>
                    ))}
                  </View>
              ...
              <TouchableOpacity style={[styles.calendarSideButton, styles.calendarSideButtonRight]} onPress={() => runMonth(1)}>
                <Text style={styles.calendarSideArrow}>›</Text>
              </TouchableOpacity>
            </View>
          </View>
        ...
        <ScrollView style={styles.listScroll} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            <View style={styles.listTitleRow}>
              <View>
                ...
                <Text style={styles.cardTitle}>
                  {memberTeamName ? `${memberTeamName} 경기 일정 관리` : "경기 일정 관리"}
                </Text>
              </View>
              <Text style={styles.listCountText}>{scheduleList.length}경기</Text>
            </View>
                ...
                <View key={item.key} style={styles.listItem}>
                  <View style={styles.listMain}>
                    ...
                    <View style={styles.listMatchRow}>
                      <Text style={styles.listOpponent}>{`${item.teamName} vs ${item.opponent}`}</Text>
                      <TouchableOpacity
                        style={[
                          styles.attendanceBadge,
                          styles.attendanceBadgeInline,
                          {
                            backgroundColor: item.attendanceStatus === "attending" ? "#d9f1df" : item.attendanceStatus === "absent" ? "#f4dada" : "#d9d5cf",
                            borderColor: item.attendanceStatus === "attending" ? "#2f8f4e55" : item.attendanceStatus === "absent" ? "#c8474755" : "#8f8f8f55",
                          },
                        ]}
                        onPress={() => setOpenedKey((current) => (current === item.key ? "" : item.key))}
                      >
                        <Text
                          style={[
                            styles.attendanceBadgeIcon,
                            { color: item.attendanceStatus === "attending" ? "#2f8f4e" : item.attendanceStatus === "absent" ? "#c84747" : "#8f8f8f" },
                          ]}
                        >
                          {item.attendanceStatus === "attending" ? "O" : item.attendanceStatus === "absent" ? "X" : "-"}
                        </Text>
                        <Text
                          style={[
                        ...
                        >
                          {item.attendanceStatus === "attending" ? "참석" : item.attendanceStatus === "absent" ? "불참" : "미응답"}
                        </Text>
                      </TouchableOpacity>
                    </View>

                    <Text style={styles.listMeta}>{item.isHome ? "홈 경기" : "원정 경기"}</Text>
                  </View>
                        ...
                        {ATTENDANCE_OPTIONS.map((option) => (
                          <TouchableOpacity
                            ...
                            style={[
                              ...
                              item.attendanceStatus === option.key && styles.attendanceOptionButtonSelected,
                            ]}
                            onPress={() => updateAttendance(item, option.key)}
                            disabled={savingKey === item.key}
                          >
                            <Text style={[styles.attendanceOptionIcon, { color: option.color }]}>{option.label}</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
                    ) : null}
                    {savingKey === item.key ? <Text style={styles.savingText}>저장 중...</Text> : null}
                  </View>
                </View>
              ))
            )}
            ...
            {scheduleList.length > visibleCount ? (
              <TouchableOpacity style={styles.moreButton} onPress={() => setVisibleCount((current) => current + 5)}>
